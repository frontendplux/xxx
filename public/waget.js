...
export async function header() {
  const user = await server_user_auth(); // wait for auth status
  // Return HTML
  return /*html*/ `
    ...
    <div class="fixed bottom-0 right-0 m-6 mr-4 text-5xl z-[1000]">
      <a href='/meet' class="relative">
        <i class='bx bxs-badge-dollar bg-red-600 text-white p-2 m-4 block rounded-full animate-bounce'>
          <span class="rounded-full p-1 bg-yellow-400 text-[small] absolute top-[-20px] m-3 right-[-10px]">23</span>
        </i>
      </a>
    </div>
    ...
    <header class="bg-[crimson] p-2 py-3 sticky top-0 shadow-md z-[1000]">
      <div class="mx-auto max-w-[1200px] w-full flex justify-between items-center">
          ...
          <nav class="flex gap-3 text-sm capitalize text-[whitesmoke] relative">
            ...
            <div class="relative">
              <button id="mobileMenuBtn" class="text-xl sm:text-sm flex items-center gap-1">
                <span class="sm:hidden">menu</span><span class="sm:inline hidden">more menu</span>
        ...
        <div class="flex items-center gap-3 text-white relative">
          <span onclick="const d=document.getElementById('searchDiv'); d.style.display=d.style.display==='none'?'block':'none'">
            <i class='bx bxs-search-alt-2 bg-white text-[crimson] p-2 py-1 rounded-full text-xl'></i>
          </span>
          ...
          <button id="userBtn" class="text-white flex items-center gap-2">
            ...
            <span class="capitalize hidden sm:inline">${user?.name || ''}</span>
          </button>
...
//     // Wait for DOM render before attaching event listeners
//     setTimeout(() => {
//       const menuBtn = document.getElementById('mobileMenuBtn');
//       const dropdown = document.getElementById('mobileDropdown');
//       const userBtn = document.getElementById('userBtn');
...
//               </button>
//               ${
//                  server_user_auth().status == true ?
//                 `<div id="userDropdown" class="absolute right-0 top-[100%] mt-2 bg-white text-black w-40 rounded shadow-md hidden z-50">
//                 <a href="/profile" class="block px-4 py-2 hover:bg-gray-100">login</a>
//                 <a href="/settings" class="block px-4 py-2 hover:bg-gray-100">signup</a>
